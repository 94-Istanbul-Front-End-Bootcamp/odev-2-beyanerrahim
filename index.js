...
var ilkharf;
if (isim != '')
    ilkharf = isim.toUpperCase.charAt(0);
else
    ilkharf = '';

function check() {

    if (input[0].checked && input[1].checked && ilkharf != '') {
        return "resit active harf";
    }
    if (input[0].checked && input[1].checked) {
        return "resit active";
...
const filterData = () => {
    ...
    switch (filter) {
        case "resit active harf":
            var filteredData = data.filter(element => element.age >= 18 && element.isActive === true && element.name.charAt(0) == ilkharf);
            listData(filteredData);
            break;
        case "resit":
            var filteredData = data.filter(element => element.age >= 18);
